...
function submitQuiz() {
    ...
    let filteredBreeds = breedData[0].filter(breed => {
        return (userAnswers.hdb === "Yes" ? breed["HDB approved"] === "Yes" : true);
    });

    // Map filtered breeds with scores
    let matches = filteredBreeds.map(breed => {
        let score = 0;
        score += Math.abs(breed.Obedience - userAnswers.trainability);
        score += Math.abs(breed.Longevity - userAnswers.longevity);
        score += Math.abs(breed["Purchase Price"] - userAnswers.initialCost) / 1000;
        score += Math.abs(breed["Food Costs"] - userAnswers.foodCost) / 100;
    ...
    topBreeds.forEach(breed => {
        ...
        breedElement.innerHTML = `
            ...
            <div>
                ...
                <progress value="${breed.Obedience}" max="100"></progress> (${breed.Obedience})
            </div>
            <div>
                <label>Longevity:</label>
                <progress value="${breed.Longevity}" max="20"></progress> (${breed.Longevity} years)
            </div>
            <div>
                <label>Purchase Price:</label>
                <progress value="${breed["Purchase Price"]}" max="5000"></progress> ($${breed["Purchase Price"]})
            </div>
            <div>
                <label>Food Costs:</label>
                <progress value="${breed["Food Costs"]}" max="2000"></progress> ($${breed["Food Costs"]})
            </div>
            <div>
                <label>Grooming Needs:</label>
                <progress value="${breed.Grooming}" max="3"></progress> (Scale: ${breed.Grooming})
            </div>
        `;
